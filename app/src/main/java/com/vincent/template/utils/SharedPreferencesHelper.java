...
/**
 * packageName:	    com.business.business.utils
 * className:	    SharedPreferencesHelper
 * author:	        Luoxiang
 * time:	        11/01/2019	11:29 AM
 * desc:
        ...
        for (Field field : fields) {
            ...
            if (value != null) {
                try {
                    ...
                    e.printStackTrace();
                    Logger.d(TAG, String.format("Set field error, Key:%s, type:%s, value:%s",
                                                key, fieldType, value));
                }
            } else {
                Logger.d(TAG, String.format("Get field value error, Key:%s, type:%s",
                                            key, fieldType));
            }
        }
